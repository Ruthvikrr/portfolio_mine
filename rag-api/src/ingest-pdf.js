...
const pdf = pdfParse.default ? pdfParse.default : pdfParse;
...
async function main() {
  ...
  if (!inputPath) {
    throw new Error("Usage: node src/ingest-pdf.js <input.pdf> [output.json]");
  }
  ...
  const cleanedFullText = data.text.replace(/([a-z])\n([a-z])/ig, "$1 $2");
